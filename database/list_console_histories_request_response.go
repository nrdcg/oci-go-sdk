...
func (request ListConsoleHistoriesRequest) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingConsoleHistorySummaryLifecycleStateEnum(string(request.LifecycleState)); !ok && request.LifecycleState != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for LifecycleState: %s. Supported values are: %s.", request.LifecycleState, strings.Join(GetConsoleHistorySummaryLifecycleStateEnumStringValues(), ",")))
	}
	if len(errMessage) > 0 {
