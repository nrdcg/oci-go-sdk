// Copyright (c) 2016, 2018, 2025, Oracle and/or its affiliates.  All rights reserved.
// This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
// Code generated. DO NOT EDIT.

// MarketplacePublisherService API
//
// Use the Marketplace Publisher API to manage the publishing of applications in Oracle Cloud Infrastructure Marketplace.
//

package marketplacepublisher

import (
	"fmt"
	"strings"

	"github.com/nrdcg/oci-go-sdk/common/v1065"
)

// AdditionalInfo Additional information about the lead
type AdditionalInfo struct {

	// The company name
	Company *string `mandatory:"false" json:"company"`

	// The company address
	CompanyAddress *string `mandatory:"false" json:"companyAddress"`

	// The email address
	Email *string `mandatory:"false" json:"email"`

	// The first name
	FirstName *string `mandatory:"false" json:"firstName"`

	// The last name
	LastName *string `mandatory:"false" json:"lastName"`

	// The phone number
	Phone *string `mandatory:"false" json:"phone"`

	// Additional notes
	Notes *string `mandatory:"false" json:"notes"`
}

func (m AdditionalInfo) String() string {
	return common.PointerString(m)
}

// ValidateEnumValue returns an error when providing an unsupported enum value
// This function is being called during constructing API request process
// Not recommended for calling this function directly
func (m AdditionalInfo) ValidateEnumValue() (bool, error) {
	errMessage := []string{}

	if len(errMessage) > 0 {
		return true, fmt.Errorf("%s", strings.Join(errMessage, "\n"))
	}
	return false, nil
}
