// Copyright (c) 2016, 2018, 2026, Oracle and/or its affiliates.  All rights reserved.
// This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
// Code generated. DO NOT EDIT.

// MarketplacePublisherService API
//
// Use the Marketplace Publisher API to manage the publishing of applications in Oracle Cloud Infrastructure Marketplace.
//

package marketplacepublisher

import (
	"fmt"
	"strings"

	"github.com/nrdcg/oci-go-sdk/common/v1065"
)

// AdminListingProduct Products associated with listing.
type AdminListingProduct struct {

	// Product Code
	Code *string `mandatory:"false" json:"code"`

	// List of category codes within a Product which are applicable to the listing revision.
	Categories []string `mandatory:"false" json:"categories"`

	// The product group for the product.
	ProductGroup *string `mandatory:"false" json:"productGroup"`

	// The product name for the product.
	ProductName *string `mandatory:"false" json:"productName"`

	// Custom filter for the product
	AdditionalFilters []ListingProductAdditionalFilter `mandatory:"false" json:"additionalFilters"`
}

func (m AdminListingProduct) String() string {
	return common.PointerString(m)
}

// ValidateEnumValue returns an error when providing an unsupported enum value
// This function is being called during constructing API request process
// Not recommended for calling this function directly
func (m AdminListingProduct) ValidateEnumValue() (bool, error) {
	errMessage := []string{}

	if len(errMessage) > 0 {
		return true, fmt.Errorf("%s", strings.Join(errMessage, "\n"))
	}
	return false, nil
}
