...
### Breaking Changes 
- The enum model `HostedApplicationLifecycleStateEnum` was renamed to `HostedApplicationBaseLifecycleStateEnum` in the Generative AI service 
- The field `InboundAuthConfig` is now required in the request model `CreateHostedApplicationDetails` in the Generative AI service
...
### Fixed  
- Fixed empty stream uploads in the Object Storage upload manager to use single-part uploads and preserve server-side encryption headers
...
### Breaking Changes 
- The model `CustomerLoggingDetails` was removed in the OpenSearch service 
- The model `ReviveWorkflowInstanceDetails` was removed in the Organizations service
...
### Breaking Changes 
- The model `UpdateComputeHostRequest` has been renamed to `UpdateComputeHostsRequest`in the Compute service 
- The model `GetComputeHostRequest` has been renamed to `GetComputeHostsRequest`in the Compute service 
...
### Breaking Changes 
- The type of `PricingTypes` has been changed from `ListingSummaryPricingTypesEnum` to `PricingTypeEnumEnum` in the model `ListingSummary` in the marketplace service
...
### Breaking Changes 
- The type of field `State` has been changed from *String to `AutoActivateStatusStateEnum` in the models `AutoActivateToggleStatus` and `AutoActivateStatus` in the Application Performance Monitoring service
