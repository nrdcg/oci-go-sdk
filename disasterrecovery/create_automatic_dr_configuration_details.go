...
// CreateAutomaticDrConfigurationDetails The details for creating an Automatic DR configuration.
type CreateAutomaticDrConfigurationDetails struct {
	...
	Members []CreateAutomaticDrConfigurationMemberDetails `mandatory:"true" json:"members"`
...
func (m CreateAutomaticDrConfigurationDetails) String() string {
...
func (m CreateAutomaticDrConfigurationDetails) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
...
func (m *CreateAutomaticDrConfigurationDetails) UnmarshalJSON(data []byte) (e error) {
	model := struct {
		...
		Members                   []createautomaticdrconfigurationmemberdetails `json:"members"`
	}{}
	...
	m.Members = make([]CreateAutomaticDrConfigurationMemberDetails, len(model.Members))
	for i, n := range model.Members {
		...
		if nn != nil {
			m.Members[i] = nn.(CreateAutomaticDrConfigurationMemberDetails)
		} else {
			m.Members[i] = nil
		}
	}
