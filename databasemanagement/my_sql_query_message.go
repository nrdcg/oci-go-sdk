...
func (m MySqlQueryMessage) String() string {
...
func (m MySqlQueryMessage) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingMySqlMessageSeverityEnum(string(m.Level)); !ok && m.Level != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for Level: %s. Supported values are: %s.", m.Level, strings.Join(GetMySqlMessageSeverityEnumStringValues(), ",")))
	}
