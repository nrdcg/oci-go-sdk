...
func (m *DatabaseConnectionDetails) UnmarshalJSON(data []byte) (e error) {
	model := struct {
		ConnectionString      databaseconnectionstringdetails `json:"connectionString"`
	...
	var nn interface{}
	nn, e = model.ConnectionString.UnmarshalPolymorphicJSON(model.ConnectionString.JsonData)
	if e != nil {
		return
	}
	if nn != nil {
		m.ConnectionString = nn.(DatabaseConnectionStringDetails)
	} else {
