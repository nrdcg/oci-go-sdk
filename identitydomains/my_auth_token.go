...
func (m MyAuthToken) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingMyAuthTokenStatusEnum(string(m.Status)); !ok && m.Status != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for Status: %s. Supported values are: %s.", m.Status, strings.Join(GetMyAuthTokenStatusEnumStringValues(), ",")))
	}
	if len(errMessage) > 0 {
...
var mappingMyAuthTokenStatusEnum = map[string]MyAuthTokenStatusEnum{
	"ACTIVE":   MyAuthTokenStatusActive,
	"INACTIVE": MyAuthTokenStatusInactive,
}

var mappingMyAuthTokenStatusEnumLowerCase = map[string]MyAuthTokenStatusEnum{
	"active":   MyAuthTokenStatusActive,
	"inactive": MyAuthTokenStatusInactive,
}
...
func GetMyAuthTokenStatusEnumValues() []MyAuthTokenStatusEnum {
	values := make([]MyAuthTokenStatusEnum, 0)
	for _, v := range mappingMyAuthTokenStatusEnum {
...
func GetMappingMyAuthTokenStatusEnum(val string) (MyAuthTokenStatusEnum, bool) {
	enum, ok := mappingMyAuthTokenStatusEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
