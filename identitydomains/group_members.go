...
func (m GroupMembers) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingGroupMembersTypeEnum(string(m.Type)); !ok && m.Type != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for Type: %s. Supported values are: %s.", m.Type, strings.Join(GetGroupMembersTypeEnumStringValues(), ",")))
	}
...
var mappingGroupMembersTypeEnum = map[string]GroupMembersTypeEnum{
	"User": GroupMembersTypeUser,
}

var mappingGroupMembersTypeEnumLowerCase = map[string]GroupMembersTypeEnum{
	"user": GroupMembersTypeUser,
}
...
func GetGroupMembersTypeEnumValues() []GroupMembersTypeEnum {
	values := make([]GroupMembersTypeEnum, 0)
	for _, v := range mappingGroupMembersTypeEnum {
...
func GetMappingGroupMembersTypeEnum(val string) (GroupMembersTypeEnum, bool) {
	enum, ok := mappingGroupMembersTypeEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
