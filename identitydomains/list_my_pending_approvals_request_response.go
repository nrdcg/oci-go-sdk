...
func (request ListMyPendingApprovalsRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingListMyPendingApprovalsSortOrderEnum(string(request.SortOrder)); !ok && request.SortOrder != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for SortOrder: %s. Supported values are: %s.", request.SortOrder, strings.Join(GetListMyPendingApprovalsSortOrderEnumStringValues(), ",")))
	}
	if len(errMessage) > 0 {
...
var mappingListMyPendingApprovalsSortOrderEnum = map[string]ListMyPendingApprovalsSortOrderEnum{
	"ASCENDING":  ListMyPendingApprovalsSortOrderAscending,
	"DESCENDING": ListMyPendingApprovalsSortOrderDescending,
}

var mappingListMyPendingApprovalsSortOrderEnumLowerCase = map[string]ListMyPendingApprovalsSortOrderEnum{
	"ascending":  ListMyPendingApprovalsSortOrderAscending,
	"descending": ListMyPendingApprovalsSortOrderDescending,
}
...
func GetListMyPendingApprovalsSortOrderEnumValues() []ListMyPendingApprovalsSortOrderEnum {
	values := make([]ListMyPendingApprovalsSortOrderEnum, 0)
	for _, v := range mappingListMyPendingApprovalsSortOrderEnum {
...
func GetMappingListMyPendingApprovalsSortOrderEnum(val string) (ListMyPendingApprovalsSortOrderEnum, bool) {
	enum, ok := mappingListMyPendingApprovalsSortOrderEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
