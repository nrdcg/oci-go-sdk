...
func (m MyCustomerSecretKey) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingMyCustomerSecretKeyStatusEnum(string(m.Status)); !ok && m.Status != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for Status: %s. Supported values are: %s.", m.Status, strings.Join(GetMyCustomerSecretKeyStatusEnumStringValues(), ",")))
	}
	if len(errMessage) > 0 {
...
var mappingMyCustomerSecretKeyStatusEnum = map[string]MyCustomerSecretKeyStatusEnum{
	"ACTIVE":   MyCustomerSecretKeyStatusActive,
	"INACTIVE": MyCustomerSecretKeyStatusInactive,
}
...
func GetMyCustomerSecretKeyStatusEnumValues() []MyCustomerSecretKeyStatusEnum {
	values := make([]MyCustomerSecretKeyStatusEnum, 0)
	for _, v := range mappingMyCustomerSecretKeyStatusEnum {
...
func GetMappingMyCustomerSecretKeyStatusEnum(val string) (MyCustomerSecretKeyStatusEnum, bool) {
	enum, ok := mappingMyCustomerSecretKeyStatusEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
