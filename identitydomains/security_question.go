...
func (m SecurityQuestion) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingSecurityQuestionTypeEnum(string(m.Type)); !ok && m.Type != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for Type: %s. Supported values are: %s.", m.Type, strings.Join(GetSecurityQuestionTypeEnumStringValues(), ",")))
	}
...
var mappingSecurityQuestionTypeEnum = map[string]SecurityQuestionTypeEnum{
	"default": SecurityQuestionTypeDefault,
	"custom":  SecurityQuestionTypeCustom,
}

var mappingSecurityQuestionTypeEnumLowerCase = map[string]SecurityQuestionTypeEnum{
	"default": SecurityQuestionTypeDefault,
	"custom":  SecurityQuestionTypeCustom,
}
...
func GetSecurityQuestionTypeEnumValues() []SecurityQuestionTypeEnum {
	values := make([]SecurityQuestionTypeEnum, 0)
	for _, v := range mappingSecurityQuestionTypeEnum {
...
func GetMappingSecurityQuestionTypeEnum(val string) (SecurityQuestionTypeEnum, bool) {
	enum, ok := mappingSecurityQuestionTypeEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
