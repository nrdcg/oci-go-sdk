...
func (m MyUserDbCredential) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingMyUserDbCredentialStatusEnum(string(m.Status)); !ok && m.Status != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for Status: %s. Supported values are: %s.", m.Status, strings.Join(GetMyUserDbCredentialStatusEnumStringValues(), ",")))
	}
	if len(errMessage) > 0 {
...
var mappingMyUserDbCredentialStatusEnum = map[string]MyUserDbCredentialStatusEnum{
	"ACTIVE":   MyUserDbCredentialStatusActive,
	"INACTIVE": MyUserDbCredentialStatusInactive,
}

var mappingMyUserDbCredentialStatusEnumLowerCase = map[string]MyUserDbCredentialStatusEnum{
	"active":   MyUserDbCredentialStatusActive,
	"inactive": MyUserDbCredentialStatusInactive,
}
...
func GetMyUserDbCredentialStatusEnumValues() []MyUserDbCredentialStatusEnum {
	values := make([]MyUserDbCredentialStatusEnum, 0)
	for _, v := range mappingMyUserDbCredentialStatusEnum {
...
func GetMappingMyUserDbCredentialStatusEnum(val string) (MyUserDbCredentialStatusEnum, bool) {
	enum, ok := mappingMyUserDbCredentialStatusEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
