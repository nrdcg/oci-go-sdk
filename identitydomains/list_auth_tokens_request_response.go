...
func (request ListAuthTokensRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingListAuthTokensSortOrderEnum(string(request.SortOrder)); !ok && request.SortOrder != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for SortOrder: %s. Supported values are: %s.", request.SortOrder, strings.Join(GetListAuthTokensSortOrderEnumStringValues(), ",")))
	}
...
var mappingListAuthTokensSortOrderEnum = map[string]ListAuthTokensSortOrderEnum{
	"ASCENDING":  ListAuthTokensSortOrderAscending,
	"DESCENDING": ListAuthTokensSortOrderDescending,
}
...
func GetListAuthTokensSortOrderEnumValues() []ListAuthTokensSortOrderEnum {
	values := make([]ListAuthTokensSortOrderEnum, 0)
	for _, v := range mappingListAuthTokensSortOrderEnum {
...
func GetMappingListAuthTokensSortOrderEnum(val string) (ListAuthTokensSortOrderEnum, bool) {
	enum, ok := mappingListAuthTokensSortOrderEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
