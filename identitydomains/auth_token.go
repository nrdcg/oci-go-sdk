...
func (m AuthToken) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingAuthTokenStatusEnum(string(m.Status)); !ok && m.Status != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for Status: %s. Supported values are: %s.", m.Status, strings.Join(GetAuthTokenStatusEnumStringValues(), ",")))
	}
	if len(errMessage) > 0 {
...
var mappingAuthTokenStatusEnum = map[string]AuthTokenStatusEnum{
	"ACTIVE":   AuthTokenStatusActive,
	"INACTIVE": AuthTokenStatusInactive,
}

var mappingAuthTokenStatusEnumLowerCase = map[string]AuthTokenStatusEnum{
	"active":   AuthTokenStatusActive,
	"inactive": AuthTokenStatusInactive,
}
...
func GetAuthTokenStatusEnumValues() []AuthTokenStatusEnum {
	values := make([]AuthTokenStatusEnum, 0)
	for _, v := range mappingAuthTokenStatusEnum {
...
func GetMappingAuthTokenStatusEnum(val string) (AuthTokenStatusEnum, bool) {
	enum, ok := mappingAuthTokenStatusEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
