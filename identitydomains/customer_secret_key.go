...
func (m CustomerSecretKey) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingCustomerSecretKeyStatusEnum(string(m.Status)); !ok && m.Status != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for Status: %s. Supported values are: %s.", m.Status, strings.Join(GetCustomerSecretKeyStatusEnumStringValues(), ",")))
	}
	if len(errMessage) > 0 {
...
var mappingCustomerSecretKeyStatusEnum = map[string]CustomerSecretKeyStatusEnum{
	"ACTIVE":   CustomerSecretKeyStatusActive,
	"INACTIVE": CustomerSecretKeyStatusInactive,
}

var mappingCustomerSecretKeyStatusEnumLowerCase = map[string]CustomerSecretKeyStatusEnum{
	"active":   CustomerSecretKeyStatusActive,
	"inactive": CustomerSecretKeyStatusInactive,
}
...
func GetCustomerSecretKeyStatusEnumValues() []CustomerSecretKeyStatusEnum {
	values := make([]CustomerSecretKeyStatusEnum, 0)
	for _, v := range mappingCustomerSecretKeyStatusEnum {
...
func GetMappingCustomerSecretKeyStatusEnum(val string) (CustomerSecretKeyStatusEnum, bool) {
	enum, ok := mappingCustomerSecretKeyStatusEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
