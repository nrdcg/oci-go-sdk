...
func (m *DatabaseToolsDatabaseApiGatewayConfigPoolCollection) UnmarshalJSON(data []byte) (e error) {
	model := struct {
		Items []databasetoolsdatabaseapigatewayconfigpoolsummary `json:"items"`
	}{}
	...
	var nn interface{}
	m.Items = make([]DatabaseToolsDatabaseApiGatewayConfigPoolSummary, len(model.Items))
	for i, n := range model.Items {
		...
		if nn != nil {
			m.Items[i] = nn.(DatabaseToolsDatabaseApiGatewayConfigPoolSummary)
		} else {
			m.Items[i] = nil
		}
	}
