...
func (m *DatabaseToolsDatabaseApiGatewayConfigPoolAutoApiSpecCollection) UnmarshalJSON(data []byte) (e error) {
	model := struct {
		Items []databasetoolsdatabaseapigatewayconfigpoolautoapispecsummary `json:"items"`
	}{}
	...
	var nn interface{}
	m.Items = make([]DatabaseToolsDatabaseApiGatewayConfigPoolAutoApiSpecSummary, len(model.Items))
	for i, n := range model.Items {
		...
		if nn != nil {
			m.Items[i] = nn.(DatabaseToolsDatabaseApiGatewayConfigPoolAutoApiSpecSummary)
		} else {
			m.Items[i] = nil
		}
	}
