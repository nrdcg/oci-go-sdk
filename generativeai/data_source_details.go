...
func (m *datasourcedetails) UnmarshalPolymorphicJSON(data []byte) (interface{}, error) {
	...
	case "DATABASE_TOOLS_CONNECTION":
		mm := DataSourceDatabaseToolsConnectionDetails{}
		err = json.Unmarshal(data, &mm)
		return mm, err
	default:
		common.Logf("Received unsupported enum value for DataSourceDetails: %s.", m.ConnectionType)
		return *m, nil
	}
}
...
var mappingDataSourceDetailsConnectionTypeEnum = map[string]DataSourceDetailsConnectionTypeEnum{
	"DATABASE_TOOLS_CONNECTION": DataSourceDetailsConnectionTypeDatabaseToolsConnection,
}

var mappingDataSourceDetailsConnectionTypeEnumLowerCase = map[string]DataSourceDetailsConnectionTypeEnum{
	"database_tools_connection": DataSourceDetailsConnectionTypeDatabaseToolsConnection,
}
...
func GetDataSourceDetailsConnectionTypeEnumValues() []DataSourceDetailsConnectionTypeEnum {
	values := make([]DataSourceDetailsConnectionTypeEnum, 0)
	for _, v := range mappingDataSourceDetailsConnectionTypeEnum {
...
func GetMappingDataSourceDetailsConnectionTypeEnum(val string) (DataSourceDetailsConnectionTypeEnum, bool) {
	enum, ok := mappingDataSourceDetailsConnectionTypeEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
