...
func (request ListApiKeysRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingApiKeyLifecycleStateEnum(string(request.LifecycleState)); !ok && request.LifecycleState != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for LifecycleState: %s. Supported values are: %s.", request.LifecycleState, strings.Join(GetApiKeyLifecycleStateEnumStringValues(), ",")))
	}
	...
	if _, ok := GetMappingListApiKeysSortByEnum(string(request.SortBy)); !ok && request.SortBy != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for SortBy: %s. Supported values are: %s.", request.SortBy, strings.Join(GetListApiKeysSortByEnumStringValues(), ",")))
	}
	if len(errMessage) > 0 {
...
var mappingListApiKeysSortOrderEnum = map[string]ListApiKeysSortOrderEnum{
	"ASC":  ListApiKeysSortOrderAsc,
	"DESC": ListApiKeysSortOrderDesc,
}
...
var mappingListApiKeysSortByEnum = map[string]ListApiKeysSortByEnum{
	"displayName": ListApiKeysSortByDisplayname,
	"timeCreated": ListApiKeysSortByTimecreated,
}

var mappingListApiKeysSortByEnumLowerCase = map[string]ListApiKeysSortByEnum{
	"displayname": ListApiKeysSortByDisplayname,
	"timecreated": ListApiKeysSortByTimecreated,
}
...
func GetListApiKeysSortByEnumValues() []ListApiKeysSortByEnum {
	values := make([]ListApiKeysSortByEnum, 0)
	for _, v := range mappingListApiKeysSortByEnum {
		values = append(values, v)
	}
	return values
}
...
func GetMappingListApiKeysSortByEnum(val string) (ListApiKeysSortByEnum, bool) {
	enum, ok := mappingListApiKeysSortByEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
