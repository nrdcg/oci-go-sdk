// Copyright (c) 2016, 2018, 2026, Oracle and/or its affiliates.  All rights reserved.
// This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
// Code generated. DO NOT EDIT.

// Generative AI Service Management API
//
// OCI Generative AI is a fully managed service that provides a set of state-of-the-art, customizable large language models (LLMs) that cover a wide range of use cases for text generation, summarization, and text embeddings.
// Use the Generative AI service management API to create and manage DedicatedAiCluster, Endpoint, Model, and WorkRequest in the Generative AI service. For example, create a custom model by fine-tuning an out-of-the-box model using your own data, on a fine-tuning dedicated AI cluster. Then, create a hosting dedicated AI cluster with an endpoint to host your custom model.
// To access your custom model endpoints, or to try the out-of-the-box models to generate text, summarize, and create text embeddings see the Generative AI Inference API (https://docs.oracle.com/iaas/api/#/en/generative-ai-inference/latest/).
// To learn more about the service, see the Generative AI documentation (https://docs.oracle.com/iaas/Content/generative-ai/home.htm).
//

package generativeai

import (
	"encoding/json"
	"fmt"
	"strings"

	"github.com/nrdcg/oci-go-sdk/common/v1065"
)

// ModelConfig Base model configuration shared across GenAI Project memory.
type ModelConfig interface {
	GetLlmSelection() LlmSelection
}

type modelconfig struct {
	JsonData        []byte
	LlmSelection    llmselection `mandatory:"true" json:"llmSelection"`
	ModelConfigType string       `json:"modelConfigType"`
}

// UnmarshalJSON unmarshals json
func (m *modelconfig) UnmarshalJSON(data []byte) error {
	m.JsonData = data
	type Unmarshalermodelconfig modelconfig
	s := struct {
		Model Unmarshalermodelconfig
	}{}
	err := json.Unmarshal(data, &s.Model)
	if err != nil {
		return err
	}
	m.LlmSelection = s.Model.LlmSelection
	m.ModelConfigType = s.Model.ModelConfigType

	return err
}

// UnmarshalPolymorphicJSON unmarshals polymorphic json
func (m *modelconfig) UnmarshalPolymorphicJSON(data []byte) (interface{}, error) {

	if data == nil || string(data) == "null" {
		return nil, nil
	}

	var err error
	switch m.ModelConfigType {
	case "CONDENSER":
		mm := CondenserConfig{}
		err = json.Unmarshal(data, &mm)
		return mm, err
	case "EXTRACTION":
		mm := ExtractionConfig{}
		err = json.Unmarshal(data, &mm)
		return mm, err
	case "EMBEDDING":
		mm := EmbeddingConfig{}
		err = json.Unmarshal(data, &mm)
		return mm, err
	default:
		common.Logf("Received unsupported enum value for ModelConfig: %s.", m.ModelConfigType)
		return *m, nil
	}
}

// GetLlmSelection returns LlmSelection
func (m modelconfig) GetLlmSelection() llmselection {
	return m.LlmSelection
}

func (m modelconfig) String() string {
	return common.PointerString(m)
}

// ValidateEnumValue returns an error when providing an unsupported enum value
// This function is being called during constructing API request process
// Not recommended for calling this function directly
func (m modelconfig) ValidateEnumValue() (bool, error) {
	errMessage := []string{}

	if len(errMessage) > 0 {
		return true, fmt.Errorf("%s", strings.Join(errMessage, "\n"))
	}
	return false, nil
}

// ModelConfigModelConfigTypeEnum Enum with underlying type: string
type ModelConfigModelConfigTypeEnum string

// Set of constants representing the allowable values for ModelConfigModelConfigTypeEnum
const (
	ModelConfigModelConfigTypeExtraction ModelConfigModelConfigTypeEnum = "EXTRACTION"
	ModelConfigModelConfigTypeEmbedding  ModelConfigModelConfigTypeEnum = "EMBEDDING"
	ModelConfigModelConfigTypeCondenser  ModelConfigModelConfigTypeEnum = "CONDENSER"
)

var mappingModelConfigModelConfigTypeEnum = map[string]ModelConfigModelConfigTypeEnum{
	"EXTRACTION": ModelConfigModelConfigTypeExtraction,
	"EMBEDDING":  ModelConfigModelConfigTypeEmbedding,
	"CONDENSER":  ModelConfigModelConfigTypeCondenser,
}

var mappingModelConfigModelConfigTypeEnumLowerCase = map[string]ModelConfigModelConfigTypeEnum{
	"extraction": ModelConfigModelConfigTypeExtraction,
	"embedding":  ModelConfigModelConfigTypeEmbedding,
	"condenser":  ModelConfigModelConfigTypeCondenser,
}

// GetModelConfigModelConfigTypeEnumValues Enumerates the set of values for ModelConfigModelConfigTypeEnum
func GetModelConfigModelConfigTypeEnumValues() []ModelConfigModelConfigTypeEnum {
	values := make([]ModelConfigModelConfigTypeEnum, 0)
	for _, v := range mappingModelConfigModelConfigTypeEnum {
		values = append(values, v)
	}
	return values
}

// GetModelConfigModelConfigTypeEnumStringValues Enumerates the set of values in String for ModelConfigModelConfigTypeEnum
func GetModelConfigModelConfigTypeEnumStringValues() []string {
	return []string{
		"EXTRACTION",
		"EMBEDDING",
		"CONDENSER",
	}
}

// GetMappingModelConfigModelConfigTypeEnum performs case Insensitive comparison on enum value and return the desired enum
func GetMappingModelConfigModelConfigTypeEnum(val string) (ModelConfigModelConfigTypeEnum, bool) {
	enum, ok := mappingModelConfigModelConfigTypeEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
