...
func (m KeySizeAlgorithm) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingAlgorithmsEnum(string(m.Name)); !ok && m.Name != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for Name: %s. Supported values are: %s.", m.Name, strings.Join(GetAlgorithmsEnumStringValues(), ",")))
	}
	if len(errMessage) > 0 {
