// Copyright (c) 2016, 2018, 2026, Oracle and/or its affiliates.  All rights reserved.
// This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
// Code generated. DO NOT EDIT.

// Queue API
//
// Use the Queue API to produce and consume messages, create queues, and manage related items. For more information, see Queue (https://docs.oracle.com/iaas/Content/queue/overview.htm).
//

package queue

import (
	"encoding/json"
	"fmt"
	"strings"

	"github.com/nrdcg/oci-go-sdk/common/v1065"
)

// LargeMessagesCapabilityDetails Specifies the details for the large messages capability.
type LargeMessagesCapabilityDetails struct {
}

func (m LargeMessagesCapabilityDetails) String() string {
	return common.PointerString(m)
}

// ValidateEnumValue returns an error when providing an unsupported enum value
// This function is being called during constructing API request process
// Not recommended for calling this function directly
func (m LargeMessagesCapabilityDetails) ValidateEnumValue() (bool, error) {
	errMessage := []string{}

	if len(errMessage) > 0 {
		return true, fmt.Errorf("%s", strings.Join(errMessage, "\n"))
	}
	return false, nil
}

// MarshalJSON marshals to json representation
func (m LargeMessagesCapabilityDetails) MarshalJSON() (buff []byte, e error) {
	type MarshalTypeLargeMessagesCapabilityDetails LargeMessagesCapabilityDetails
	s := struct {
		DiscriminatorParam string `json:"type"`
		MarshalTypeLargeMessagesCapabilityDetails
	}{
		"LARGE_MESSAGES",
		(MarshalTypeLargeMessagesCapabilityDetails)(m),
	}

	return json.Marshal(&s)
}
