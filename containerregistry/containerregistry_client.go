...
func NewContainerRegistryClientWithConfigurationProvider(configProvider common.ConfigurationProvider) (client ContainerRegistryClient, err error) {
	if enabled := common.CheckForEnabledServices("containerregistry"); !enabled {
	...
	return newContainerRegistryClientFromBaseClient(baseClient, provider)
}
...
func newContainerRegistryClientFromBaseClient(baseClient common.BaseClient, configProvider common.ConfigurationProvider) (client ContainerRegistryClient, err error) {
	// ContainerRegistry service default circuit breaker is enabled
	baseClient.Configuration.CircuitBreaker = common.NewCircuitBreaker(common.DefaultCircuitBreakerSettingWithServiceName("ContainerRegistry"))
	common.ConfigCircuitBreakerFromEnvVar(&baseClient)
	common.ConfigCircuitBreakerFromGlobalVar(&baseClient)

	client = ContainerRegistryClient{BaseClient: baseClient}
	client.BasePath = "20180419"
...
func (client *ContainerRegistryClient) SetRegion(region string) {
	client.Host, _ = common.StringToRegion(region).EndpointForTemplateDottedRegion("containerregistry", "https://ocir.{region}.oci.{secondLevelDomain}", "ocir")
}
...
// # See also
//
// Click https://docs.oracle.com/en-us/iaas/tools/go-sdk-examples/latest/containerregistry/GetAccessToken.go.html to see an example of how to use GetAccessToken API.
// A default retry strategy applies to this operation GetAccessToken()
func (client ContainerRegistryClient) GetAccessToken(ctx context.Context, request GetAccessTokenRequest) (response GetAccessTokenResponse, err error) {
	...
	ociResponse, err = common.Retry(ctx, request, client.getAccessToken, policy)
	if err != nil {
		if ociResponse != nil {
			if httpResponse := ociResponse.HTTPResponse(); httpResponse != nil {
				opcRequestId := httpResponse.Header.Get("opc-request-id")
				response = GetAccessTokenResponse{RawResponse: httpResponse, OpcRequestId: &opcRequestId}
			} else {
	...
	if convertedResponse, ok := ociResponse.(GetAccessTokenResponse); ok {
		response = convertedResponse
	} else {
		err = fmt.Errorf("failed to convert OCIResponse into GetAccessTokenResponse")
	}
	return
}

// getAccessToken implements the OCIOperation interface (enables retrying operations)
func (client ContainerRegistryClient) getAccessToken(ctx context.Context, request common.OCIRequest, binaryReqBody *common.OCIReadSeekCloser, extraHeaders map[string]string) (common.OCIResponse, error) {

	httpRequest, err := request.HTTPRequest(http.MethodGet, "/docker/token", binaryReqBody, extraHeaders)
	if err != nil {
		return nil, err
	}
