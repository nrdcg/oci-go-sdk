...
func (m *GenerateTextDetails) UnmarshalJSON(data []byte) (e error) {
	model := struct {
		...
		InferenceRequest llminferencerequest `json:"inferenceRequest"`
	}{}
	...
	nn, e = model.InferenceRequest.UnmarshalPolymorphicJSON(model.InferenceRequest.JsonData)
	if e != nil {
		return
	}
	if nn != nil {
		m.InferenceRequest = nn.(LlmInferenceRequest)
	} else {
		m.InferenceRequest = nil
	}
