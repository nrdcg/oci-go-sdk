...
func (m *DatabaseToolsKeyStorePostgresqlSummary) UnmarshalJSON(data []byte) (e error) {
	model := struct {
		...
		KeyStoreContent  databasetoolskeystorecontentpostgresqlsummary  `json:"keyStoreContent"`
		KeyStorePassword databasetoolskeystorepasswordpostgresqlsummary `json:"keyStorePassword"`
	}{}
	...
	if nn != nil {
		m.KeyStoreContent = nn.(DatabaseToolsKeyStoreContentPostgresqlSummary)
	} else {
	...
	if nn != nil {
		m.KeyStorePassword = nn.(DatabaseToolsKeyStorePasswordPostgresqlSummary)
	} else {
