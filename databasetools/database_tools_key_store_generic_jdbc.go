...
func (m *DatabaseToolsKeyStoreGenericJdbc) UnmarshalJSON(data []byte) (e error) {
	model := struct {
		...
		KeyStoreContent  databasetoolskeystorecontentgenericjdbc  `json:"keyStoreContent"`
		KeyStorePassword databasetoolskeystorepasswordgenericjdbc `json:"keyStorePassword"`
	}{}
	...
	if nn != nil {
		m.KeyStoreContent = nn.(DatabaseToolsKeyStoreContentGenericJdbc)
	} else {
	...
	if nn != nil {
		m.KeyStorePassword = nn.(DatabaseToolsKeyStorePasswordGenericJdbc)
	} else {
