...
func (m *DatabaseToolsKeyStorePostgresql) UnmarshalJSON(data []byte) (e error) {
	model := struct {
		...
		KeyStoreContent  databasetoolskeystorecontentpostgresql  `json:"keyStoreContent"`
		KeyStorePassword databasetoolskeystorepasswordpostgresql `json:"keyStorePassword"`
	}{}
	...
	if nn != nil {
		m.KeyStoreContent = nn.(DatabaseToolsKeyStoreContentPostgresql)
	} else {
	...
	if nn != nil {
		m.KeyStorePassword = nn.(DatabaseToolsKeyStorePasswordPostgresql)
	} else {
