...
func (m *DatabaseToolsDatabaseApiGatewayConfigCollection) UnmarshalJSON(data []byte) (e error) {
	model := struct {
		Items []databasetoolsdatabaseapigatewayconfigsummary `json:"items"`
	}{}
	...
	var nn interface{}
	m.Items = make([]DatabaseToolsDatabaseApiGatewayConfigSummary, len(model.Items))
	for i, n := range model.Items {
		...
		if nn != nil {
			m.Items[i] = nn.(DatabaseToolsDatabaseApiGatewayConfigSummary)
		} else {
			m.Items[i] = nil
		}
	}
