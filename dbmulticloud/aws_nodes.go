...
// AwsNodes AWS Identity Connector Node Details.
type AwsNodes struct {
	...
	Status AwsNodesStatusEnum `mandatory:"false" json:"status,omitempty"`
...
func (m AwsNodes) ValidateEnumValue() (bool, error) {
	errMessage := []string{}

	if _, ok := GetMappingAwsNodesStatusEnum(string(m.Status)); !ok && m.Status != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for Status: %s. Supported values are: %s.", m.Status, strings.Join(GetAwsNodesStatusEnumStringValues(), ",")))
	}
	if len(errMessage) > 0 {
...
// AwsNodesStatusEnum Enum with underlying type: string
type AwsNodesStatusEnum string

// Set of constants representing the allowable values for AwsNodesStatusEnum
const (
	AwsNodesStatusConnected    AwsNodesStatusEnum = "CONNECTED"
	AwsNodesStatusDisconnected AwsNodesStatusEnum = "DISCONNECTED"
	AwsNodesStatusUnknown      AwsNodesStatusEnum = "UNKNOWN"
)

var mappingAwsNodesStatusEnum = map[string]AwsNodesStatusEnum{
	"CONNECTED":    AwsNodesStatusConnected,
	"DISCONNECTED": AwsNodesStatusDisconnected,
	"UNKNOWN":      AwsNodesStatusUnknown,
}

var mappingAwsNodesStatusEnumLowerCase = map[string]AwsNodesStatusEnum{
	"connected":    AwsNodesStatusConnected,
	"disconnected": AwsNodesStatusDisconnected,
	"unknown":      AwsNodesStatusUnknown,
}

// GetAwsNodesStatusEnumValues Enumerates the set of values for AwsNodesStatusEnum
func GetAwsNodesStatusEnumValues() []AwsNodesStatusEnum {
	values := make([]AwsNodesStatusEnum, 0)
	for _, v := range mappingAwsNodesStatusEnum {
		values = append(values, v)
	}
	return values
}

// GetAwsNodesStatusEnumStringValues Enumerates the set of values in String for AwsNodesStatusEnum
func GetAwsNodesStatusEnumStringValues() []string {
	return []string{
		"CONNECTED",
		"DISCONNECTED",
		"UNKNOWN",
...
// GetMappingAwsNodesStatusEnum performs case Insensitive comparison on enum value and return the desired enum
func GetMappingAwsNodesStatusEnum(val string) (AwsNodesStatusEnum, bool) {
	enum, ok := mappingAwsNodesStatusEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
