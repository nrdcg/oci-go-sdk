...
// # See also
//
// Click https://docs.oracle.com/en-us/iaas/tools/go-sdk-examples/latest/datasafe/ListAssociatedResources.go.html to see an example of how to use ListAssociatedResourcesRequest.
type ListAssociatedResourcesRequest struct {
	...
	AssociatedResourceType AssociatedResourceSummaryAssociatedResourceTypeEnum `mandatory:"false" contributesTo:"query" name:"associatedResourceType" omitEmpty:"true"`
...
func (request ListAssociatedResourcesRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingAssociatedResourceSummaryAssociatedResourceTypeEnum(string(request.AssociatedResourceType)); !ok && request.AssociatedResourceType != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for AssociatedResourceType: %s. Supported values are: %s.", request.AssociatedResourceType, strings.Join(GetAssociatedResourceSummaryAssociatedResourceTypeEnumStringValues(), ",")))
	}
	if len(errMessage) > 0 {
